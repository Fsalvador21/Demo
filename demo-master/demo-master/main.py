...
class Game:
    def __init__(self):
        ...
        pg.display.set_caption("Egg Bunny Jumps")
    ...
    def load_data(self):
        ...
        self.boost_sound = pg.mixer.Sound(path.join(self.snd_dir, 'Jump24.wav'))
        self.head_jump_sound = pg.mixer.Sound(path.join(self.snd_dir, 'Bounce.ogg'))
    def new(self):
        self.score = 0
        ...
        for i in range(8):
            ...
            c.rect.y += 500
        # load music
        pg.mixer.music.load(path.join(self.snd_dir, 'BeepBox-Song.wav'))
        # call the run method
        self.run()
    def run(self):
        # game loop
